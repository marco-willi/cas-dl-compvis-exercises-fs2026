"""Step 4 — Download and resize SER images.

Reads ser_manifest.json, downloads each image from Azure blob storage,
resizes to max 1024 px on the longer side (aspect-ratio preserving, Lanczos),
and saves as JPEG quality 92.

Writes:
    data/ser/<split>/<label>/<image_id>.jpg
    data/ser/metadata.csv
    data/ser/ser_failed.txt  (only if failures occur)

Usage:
    python scripts/prepare_datasets/ser/04_download_images.py
"""
...
OUTPUT_DIR = DATA_DIR / "ser"

MAX_SIZE = 1024  # max side length in pixels
JPEG_QUALITY = 92
MAX_WORKERS = 8
MAX_RETRIES = 3
...
# ---------------------------------------------------------------------------
# Download + resize one image
# ---------------------------------------------------------------------------
def process_one(entry: dict, output_dir: Path) -> dict:
    """Download, resize and save one image. Returns a result dict."""
    split = entry["split"]
    label = entry["label"]
    url = entry["url"]
    image_id = entry["image_id"]

    # Use the basename stem to get a flat filename (image_id contains slashes)
    dest_dir = output_dir / split / label
    dest_dir.mkdir(parents=True, exist_ok=True)
    dest_path = dest_dir / f"{Path(image_id).stem}.jpg"

    if dest_path.exists():
        # Read existing dimensions from file
        try:
...
with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
    ...
    for done_count, future in enumerate(as_completed(futures), start=1):
        ...
        if done_count % 50 == 0 or done_count == len(manifest):
...
if failed:
    fail_path = OUTPUT_DIR / "ser_failed.txt"
