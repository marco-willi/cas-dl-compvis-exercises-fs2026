"""Step 5 — Generate cropped-animal images using MegaDetector bounding boxes.

For each downloaded image with a non-null md_bbox, crops the detected animal
region with 10% padding on each side (clamped to image bounds), then resizes
the crop to max 1024 px on the longer side.

Skips empty-class images (no bounding box).

Writes:
    data/ser_cropped/<split>/<label>/<image_id>.jpg
    data/ser_cropped/metadata_cropped.csv

Usage:
    python scripts/prepare_datasets/ser/05_crop_images.py
"""
...
MANIFEST_PATH = SCRIPT_DIR / "ser_manifest.json"
SRC_DIR = DATA_DIR / "ser"
OUT_DIR = DATA_DIR / "ser_cropped"

MAX_SIZE = 1024
JPEG_QUALITY = 92
PADDING_FRAC = 0.10  # 10% padding on each side
...
def crop_with_padding(img: Image.Image, bbox_norm: list[float]) -> Image.Image:
    """Crop img to bbox (normalised [x, y, w, h]) with proportional padding.

    Args:
        img: PIL image (RGB)
        bbox_norm: normalised [x, y, w, h] from MegaDetector
...
def resize_if_needed(img: Image.Image) -> Image.Image:
    """Resize to max MAX_SIZE on the longer side if needed."""
    w, h = img.size
    if max(w, h) <= MAX_SIZE:
        return img
    if w >= h:
...
# Only process entries that have a bounding box
...
# Directory structure
print("\nDirectory structure (ser_cropped):")
